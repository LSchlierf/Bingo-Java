...
public class BingoCard {
    ...
    /**
     * Creates a new BingoCard form the BinogTile array. Constructor is set to private 
     * to insure only square BingoCards are created, use static methods to obtain BingoCards.
    ...
    /**
     * Replaces the center BingoTile with a FreeBingoTile. Only works when the side length 
     * is an odd number, so the BingoCard has an actual center.
     */
    public void addFreeTile(){
        if(getSize() % 2 == 1) tiles[tiles.length / 2][tiles[0].length / 2] = new FreeBingoTile();
    }
    ...
    public static BingoCard createFromStringArray(String[][] texts){
        ...
        return new BingoCard(Arrays.stream(texts)
                                    .map(a -> Arrays.stream(a)
                                        .map(s -> new BingoTile(s))
     ...
     * @return {@code null} if the BingoSet doesn't contain contain enough entries, or the new BingoCard otherwise.
     * @throws IOException if an IOException occurs while fetching the BingoSet
     */
    public static BingoCard createFromSet(String setName ,int size) throws IOException{
        ...
        return createFromStringArray(texts);
    }

    /**
     * Creates a new BingoCard with size 5 using {@code createFromSet(setName, size)}.
     * @param setName the name of the BingoSet
     * @return {@code null} if the BingoSet doesn't contain contain enough entries, or the new BingoCard otherwise.
     * @throws IOException if an IOException occurs while fetching the BingoSet
     */
    public static BingoCard createFromSet(String setName) throws IOException{
        return createFromSet(setName, 5);
    }
    ...
    /**
     * Formats this BingoCard for console print, leaving out whether the BingoTiles have been marked off.
    ...
    /**
     * Formats this BinogCard for console output, also displaying which BingoTiles have been marked off.
     * @return a nicely formatted String version of this BingoCard
     */
