...
public class BingoSets {
     ...
     * @return {@code true} if the BingoSet could be created, {@code false} otherwise
     ...
     * @param name the name of the BingoSet
     * @return {@code true} if the BingoSet could be deleted, {@code false} otherwise
     */
    ...
    /**
     * Throws out all entries in the specified BingoSet and enters the specified entries.
     * @param name the name of the BingoSet
    ...
    public static boolean changeSet(String name, String... texts) throws IOException{
        if(!deleteSet(fetchPath(name))) return false;
